...
def ask_music():
    ...
    if question == "y":
        ...
        update_callback("\033[34m" + "continuing without music..." + "\033[0m")
    else:
...
def initialize_hack():
    ...
    for letter in intro:
        text += letter
        introduction("\033[34m" + text + "\033[0m")
        time.sleep(0.1)
    time.sleep(1.0)
    subprocess.Popen(command, shell=True)
    time.sleep(1.0)
    target = input("\033[35m" + "enter targer name: " + "\033[0m")
    if target == "":
        initialize_hack()
    else:
        do_hacking(target)
